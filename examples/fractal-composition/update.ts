...
import { program as CounterApp , type Model as CounterModel} from "./apps/counter/program";
import { program as TodoApp , type Model as TodoModel} from "./apps/todo/program";
import { program as WeatherApp , type Model as WeatherModel} from "./apps/weather/program";

export type Msg =
  | { type: "Counter"; msg: any }
  | { type: "Todo"; msg: any }
  | { type: "Weather"; msg: any }
  | { type: "REFRESH_ALL" }
  | { type: "ALL_FETCHED"; results: any[] };

export const update = (msg: Msg, m: Model, dispatch: any) => {
  switch (msg.type) {
    case "REFRESH_ALL": {
    ...
    case "Counter": {
      const { model: counter } = CounterApp.update(msg.msg, m.counter, (sub: CounterModel) =>
    ...
    case "Todo": {
      const { model: todo } = TodoApp.update(msg.msg, m.todo, (sub: TodoModel) =>
    ...
    case "Weather": {
      const { model: weather } = WeatherApp.update(msg.msg, m.weather, (sub: WeatherModel) =>
