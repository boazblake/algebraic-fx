...
export const view = (m: Model, dispatch: (msg: Msg) => void) => {
  ...
  return div(
    {
      className:
        "flex flex-col space-y-4 p-4 bg-white shadow rounded-lg max-w-sm w-full mx-auto transition",
    },
    [
      ...
      div({ className: "flex space-x-2" }, [
        input({
          className:
            "flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none",
          value: m.city,
