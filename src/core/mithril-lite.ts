...
function updateNodes(
  ...
  const newIndexToOldIndex = new Array<number>(newLen);
  for (let i = 0; i < newLen; i++) newIndexToOldIndex[i] = -1;
...
function setAttr(
  ...
  if (key === "checked") {
    if (dom.checked !== !!value) dom.checked = !!value;
    return;
  }

  if (key === "selected") {
    if (dom.selected !== !!value) dom.selected = !!value;
    return;
  }
